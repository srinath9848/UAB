ALTER PROCEDURE [dbo].[Rpt_GenerateChartSummaryReportDetails] (@ProjectId INT,@CurrentDos date,@ColumnName varchar(50), @DateType varchar(15)) 
AS                
BEGIN
...
BEGIN          
SELECT distinct CC.DateOfService AS [Date],L.Name AS ListName,CC.ClinicalCaseId AS ClinicalCaseId    
...
IF @ColumnName = 'Received'            
BEGIN          
SELECT distinct CONVERT(DATE,cc.CreatedDate,101) AS [Date],L.Name AS ListName,CC.ClinicalCaseId AS ClinicalCaseId    
 ,CC.PatientMRN AS PatientMRN, CC.PatientFirstName+' '+CC.PatientLastName AS Name,ISNULL(Pro.Name,PP.Name) AS Provider    
FROM ClinicalCase cc              
JOIN WorkItem wi              
ON wi.ClinicalCaseId = cc.ClinicalCaseId              
...
LEFT JOIN List L ON L.ListId = CC.ListId    
 ...
 LEFT JOIN Provider PP ON PP.ProviderID = Wip.ProviderId
WHERE cc.ProjectId = @ProjectId              
AND CONVERT(DATE,cc.CreatedDate,101) = @CurrentDos             
END            
ELSE IF @ColumnName = 'Coded'            
BEGIN          
SELECT distinct CONVERT(DATE,cc.CreatedDate,101) AS [Date],L.Name AS ListName,CC.ClinicalCaseId AS ClinicalCaseId    
