...
where    
wi.AssignedTo=@UserId and wi.projectid=@ProjectID and wi.QABy is NULL and wi.ShadowQABy is NULL and wi.IsBlocked=1 and wi.StatusId=2    
...
LEFT JOIN List l ON l.ListId = CC.ListId      
where    
wi.QABy=@UserId and wi.projectid=@ProjectID and wi.ShadowQABy is NULL and wi.IsBlocked=1 and wi.StatusId=5    
order by bh.BlockHistoryid desc    
  END    
...
LEFT JOIN List l ON l.ListId = CC.ListId        
where    
wi.projectid=@ProjectID and wi.ShadowQABy=@UserId and wi.IsBlocked=1 and wi.StatusId=9    
order by bh.BlockHistoryid desc    
  END    
END 