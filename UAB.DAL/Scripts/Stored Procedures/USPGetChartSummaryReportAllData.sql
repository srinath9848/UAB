...
;WITH searchcte as(                               
 ...
 CC.ClinicalCaseID,    
  ...
  p.ProjectId     
  ,PF.Feedback    
 , CASE WHEN wi.StatusId=17 THEN  pr.Name WHEN wi.StatusId=16 THEN dbo.fn_GetUserName(wi.AssignedTo) ELSE '' END AS PostedByName         
 ,wi.PostingDate    
...
select *, dbo.fn_GetUserName(AssignedTo) AS CodedByName              
  ,dbo.fn_GetUserName(QABy) AS QAByName                                        
    ,dbo.fn_GetUserName(ShadowQABy) AS ShadowQAByName
	from #Result  
