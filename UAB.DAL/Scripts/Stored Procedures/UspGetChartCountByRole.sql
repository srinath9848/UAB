...
Create procedure [dbo].[UspGetChartCountByRole] --19,'Coder'  
@UserId INT,              
@Role VARCHAR(100)                 
AS                  
BEGIN                  
...
BEGIN                
 SELECT P.ProjectId,P.[Name],                
 SUM(CASE WHEN StatusId = 4 THEN 1 ELSE 0 END) + SUM(CASE WHEN StatusId = 5 AND ISNULL(IsBlocked, 0) = 0 AND QABy = 1 THEN 1 ELSE 0 END) AvailableCharts,                
 ...
 SUM(CASE WHEN AssignedTo = @UserId AND StatusId = 5 AND ISNULL(IsBlocked, 0) = 1 THEN 1 ELSE 0 END) BlockedCharts,       
...
BEGIN                
 SELECT P.ProjectId,P.Name,                
 SUM(CASE WHEN StatusId = 8 THEN 1 ELSE 0 END) + SUM(CASE WHEN StatusId = 9 AND ShadowQABy = @UserId THEN 1 ELSE 0 END) AvailableCharts,                
 SUM(CASE WHEN StatusId = 13 AND wi.ShadowQABy = @UserId THEN 1 ELSE 0 END) IncorrectCharts,                
 SUM(CASE WHEN AssignedTo = @UserId AND StatusId = 9 AND ISNULL(IsBlocked, 0) = 1 THEN 1 ELSE 0 END) BlockedCharts,        
 0 AS ReadyForPostingCharts,                
 0 AS CoderRebuttalCharts,                
 0 AS ShadowQARebuttalCharts                
 FROM Project p  
