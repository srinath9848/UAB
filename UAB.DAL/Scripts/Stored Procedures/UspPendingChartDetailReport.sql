...
BEGIN          
 ...
 LEFT JOIN Provider PP ON PP.ProviderID = Wip.ProviderId
 WHERE W.ProjectId = @ProjectId  
 AND DATEADD(Second, @TimeZoneOffset * 60 * 60, BH.CreateDate) >= @WeekStartDate    
AND  DATEADD(Second, @TimeZoneOffset * 60 * 60, BH.CreateDate) <= DATEADD(Hour,23,DATEADD(MINUTE,59,DATEADD(SECOND,59,@WeekEndDate)))
END      
