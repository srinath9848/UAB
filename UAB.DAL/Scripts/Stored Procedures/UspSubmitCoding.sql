ALTER PROCEDURE [dbo].[UspSubmitCoding]  --1,'some text',1,'71045',26,'J98.4',1,1,1,null                              
@PayorID INT = NULL,                              
...
BEGIN TRY                                    
 ...
 IF EXISTS(Select 1 from WorkItem where ClinicalCaseId=@ClinicalcaseID AND AssignedTo=@UserId AND StatusId=2)  
 BEGIN
...
END                               
 ...
 if @StatusId = 16  
 update workitem set postingdate = GETUTCDATE() WHERE ClinicalCaseId = @ClinicalcaseID       
...
ELSE
   BEGIN        
   declare @err1 varchar(max)= (select  'This chart is no longer available in the coding queue'  )                              
