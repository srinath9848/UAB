CREATE PROCEDURE [dbo].[USPGetSearchData]       
@userId int = null,    
...
;WITH searchcte as(                       
	...
	p.[Name] as ProjectName, s.[Name] as [Status]  ,wi.IsBlocked  ,wi.AssignedTo,wi.QABy,wi.ShadowQABy,  
	p.ProjectId  
	,ISNULL(pr.Name, ISNULL(pro.Name ,procc.Name)) as [Provider] 
	...
	AND (@providername IS NULL OR (pro.[Name] = @providername OR procc.[Name] = @providername))    
	AND (@StatusName IS NULL OR s.[Name] = @StatusName)
	AND ISNULL(wi.IsBlocked, 0) = @IsBlocked
)         
SELECT * FROM searchcte   
WHERE providerorder=1  AND (dbo.fnIsManager(ProjectId,@userId) = 1 OR (AssignedTo=@userId OR QABy=@userId OR ShadowQABy=@userId))  
END