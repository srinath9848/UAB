CREATE PROCEDURE [dbo].[UspBacklogChartReport] --NULL,'PerDay'            
@ProjectId INT = NULL,            
...
count(cc.ClinicalCaseID) AS Total        
 ,DATEDIFF(DAY,cc.DateOfService,GETUTCDATE())As DelayDays        
...
where        
wi.StatusId <>16 AND DATEDIFF(DAY,cc.DateOfService,GETUTCDATE())>=p.SLAInDays        
and p.ProjectId=@ProjectId        
--and  DATEDIFF(DAY,cc.DateOfService,GETUTCDATE()) =1  
...
group by DATEDIFF(DAY,cc.DateOfService,GETUTCDATE())        
,s.Name,p.Name         
END        
...
wi.StatusId <>16 AND DATEDIFF(DAY,cc.DateOfService,GETUTCDATE())>=p.SLAInDays        
and p.ProjectId=@ProjectId         
and  DATEDIFF(DAY,cc.DateOfService,GETUTCDATE()) <=30  
AND (cc.DateOfService >= @StartDate OR (cc.DateOfService = ISNULL(@StartDate,cc.DateOfService)))  
 AND (cc.DateOfService <= @EndDate OR (cc.DateOfService = ISNULL(@EndDate,cc.DateOfService)))  
group by DATEDIFF(DAY,cc.DateOfService,GETUTCDATE())        
,s.Name,p.Name         
