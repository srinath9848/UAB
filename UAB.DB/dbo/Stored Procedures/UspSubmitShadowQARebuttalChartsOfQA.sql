...
IF NOT EXISTS (     
 SELECT FieldName, FieldValue, ClaimId, IsAccepted FROM WorkItemAudit   WHERE ClinicalCaseId = @ClinicalcaseID AND VersionId = (   SELECT MIN(VersionId) FROM WorkItemAudit WHERE ClinicalCaseId = @ClinicalcaseID)  EXCEPT
   SELECT FieldName, FieldValue, ClaimId, IsAccepted FROM @utWorkItemAudit )  
SET @StatusID = 15  
